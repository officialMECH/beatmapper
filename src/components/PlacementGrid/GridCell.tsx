...
import { type App, type ObjectPlacementMode, type ObjectSelectionMode, ObjectTool } from "$/types";
...
interface Props {
	...
	setMouseDownAt: Dispatch<SetStateAction<{ rowIndex: number; colIndex: number; x: number; y: number } | null>>;
	setMouseOverAt: Dispatch<SetStateAction<{ rowIndex: number; colIndex: number; x?: number; y?: number } | null>>;
	mappingMode: ObjectPlacementMode;
	defaultObstacleDuration: number;
	selectionMode: ObjectSelectionMode | null;
	selectedTool: ObjectTool;
	setHoveredCell: Dispatch<SetStateAction<{ rowIndex: number; colIndex: number; x?: number; y?: number } | null>>;
	clickPlacementGrid: (rowIndex: number, colIndex: number) => void;
	createNewObstacle: (data: App.Obstacle) => void;
}
