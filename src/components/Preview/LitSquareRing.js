...
const RingPeg = ({ size, thickness, color, zRotation, lightStatus, lightColor, lastLightingEventId, isPlaying }) => {
	const length = size;
	...
	return (
		<group rotation={[0, 0, zRotation]}>
			...
			<mesh position={[0, length / 2 - width / 2 - thickness - 0.1, 0]} rotation={[Math.PI * 0.5, 0, 0]}>
				...
				<animated.meshLambertMaterial attach="material" emissive={lightColor} transparent={true} side={THREE.DoubleSide} {...lightingSpring} />
			</mesh>
		</group>
	);
};

const LitSquareRing = ({ size = 12, thickness, x = 0, y = -2, z = -8, color, rotation, lightStatus, lightColor, lastLightingEventId, isPlaying }) => {
