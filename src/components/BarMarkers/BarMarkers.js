...
const BarMarkers = ({ duration, bpm, cursorPositionInBeats, beatDepth, graphicsLevel }) => {
	...
	const linesArray = React.useMemo(() => range(totalNumOfBeats * 4), [totalNumOfBeats]);
	const visibleSubsetOfLines = linesArray.filter((i) => {
		...
		return beat >= cursorPositionInBeats - FUDGE_FACTOR && beat < cursorPositionInBeats + numToRender + FUDGE_FACTOR;
	});
	...
	return visibleSubsetOfLines.map((i) => {
		...
		return <Marker key={i} beatNum={isBeat && i / 4} offset={-SONG_OFFSET + -i * (beatDepth / 4)} type={type} />;
	});
};
