...
const jumpToEarliestNote = (earlierNotes, laterNotes, earlierObstacles, laterObstacles, store) => {
	...
	const relevantObstacles = findUniquesWithinArrays(earlierObstacles, laterObstacles);
	...
	const [closeLimit, farLimit] = calculateVisibleRange(cursorPositionInBeats, beatDepth, graphicsLevel);

	const entityTime = typeof earliestEntity._time === "number" ? earliestEntity._time : earliestEntity.beatStart;
	...
	return (store) => (next) => (action) => {
		switch (action.type) {
			case "UNDO_NOTES": {
				...
				jumpToEarliestNote(pastNotes, presentNotes, pastObstacles, presentObstacles, store);
			...
			case "REDO_NOTES": {
				...
				jumpToEarliestNote(presentNotes, futureNotes, presentObstacles, futureObstacles, store);
