...
const notes = (state = initialState.notes, action = undefined) => {
	switch (action.type) {
      ...
      */
			...
			const earliestBeat = isBlockOrMine(data[0]) ? data[0]._time : data[0].beatStart;
...
const obstacles = (state = initialState.obstacles, action = undefined) => {
	switch (action.type) {
		case "CREATE_NEW_SONG":
		case "START_LOADING_SONG":
		case "LEAVE_EDITOR": {
		...
		case "PASTE_SELECTION": {
			...
			const earliestBeat = isObstacle(data[0]) ? data[0].beatStart : data[0]._time;
			const deltaBetweenPeriods = pasteAtBeat - earliestBeat;
...
export const getObstacles = (state) => state.editorEntities.notesView.present.obstacles;
...
export const getVisibleNotes = createSelector(getNotes, getCursorPositionInBeats, getBeatDepth, getGraphicsLevel, (notes, cursorPositionInBeats, beatDepth, graphicsLevel) => {
	const [closeLimit, farLimit] = calculateVisibleRange(cursorPositionInBeats, beatDepth, graphicsLevel, { includeSpaceBeforeGrid: true });
