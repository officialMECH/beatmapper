...
export const zipFiles = async (song, songFile, coverArtFile, version) => {
	const zip = new JSZip();
...
export const convertLegacyArchive = async (archive) => {
	...
	const infoDatString = await getFileFromArchive(archive, "info.json").async("string");
	...
	const coverImageFile = await getFileFromArchive(archive, infoDatJson.coverImagePath).async("blob");
	// TODO: Support PNG?
	...
	const loadedDifficultyFiles = await Promise.all(
		infoDatJson.difficultyLevels.map(async (level) => {
			const fileContents = await getFileFromArchive(archive, level.jsonPath).async("string");
...
export const processImportedMap = async (zipFile, currentSongIds) => {
	...
	const infoDatString = await getFileFromArchive(archive, "Info.dat").async("string");
	const infoDatJson = JSON.parse(infoDatString);
	...
	const uncompressedSongFile = await getFileFromArchive(archive, infoDatJson._songFilename).async("blob");
	const uncompressedCoverArtFile = await getFileFromArchive(archive, infoDatJson._coverImageFilename).async("blob");
	...
	const beatmapSet = infoDatJson._difficultyBeatmapSets.find((set) => set._beatmapCharacteristicName === "Standard");

	// We do check if a lightshow exists only so we can store that setting, to
	// include lightmaps when exporting
	const enabledLightshow = infoDatJson._difficultyBeatmapSets.some((set) => set._beatmapCharacteristicName === "Lightshow");
	...
	const difficultiesById = difficultyFiles.reduce((acc, { id, noteJumpSpeed, startBeatOffset, customLabel, ...rest }) => {
		return {
			...acc,
			[id]: {
				id,
