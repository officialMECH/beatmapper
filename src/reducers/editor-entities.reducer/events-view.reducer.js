...
const eventsView = undoable(
		...
		switch (action.type) {
			...
			case "PLACE_EVENT": {
				...
				return produce(state, (draftState) => {
					...
					if (areLasersLocked && trackId === App.TrackId[2]) {
						...
						draftState.tracks[mirrorTrackId].splice(indexToInsertAt, 0, symmetricalEvent);
					}
				});
			}
			...
			case "CHANGE_LASER_SPEED": {
				...
				return produce(state, (draftState) => {
					...
					draftState.tracks[trackId].splice(indexToInsertAt, numToRemove, newEvent);
					...
					if (areLasersLocked && trackId === App.TrackId[12]) {
						...
						const [indexToInsertAt, eventOverlaps] = findIndexForNewEvent(beatNum, relevantEvents);
						const numToRemove = eventOverlaps ? 1 : 0;
						draftState.tracks[symmetricalTrackId].splice(indexToInsertAt, numToRemove, symmetricalEvent);
					}
				});
			}
			...
			case "BULK_DELETE_EVENT": {
				...
				return produce(state, (draftState) => {
					...
					if (areLasersLocked && mirroredTracks.includes(trackId)) {
						...
						draftState.tracks[mirroredTrackId] = draftState.tracks[mirroredTrackId].filter((ev) => ev.id !== getSymmetricalId(id));
					}
				});
			}
			...
			case "DESELECT_EVENT": {
				...
				return produce(state, (draftState) => {
					draftState.tracks[trackId][eventIndex].selected = action.type === "SELECT_EVENT";
				});
			}
	...
	},
	{
		...
		filter: includeAction(["PLACE_EVENT", "CHANGE_LASER_SPEED", "DELETE_EVENT", "DELETE_SELECTED_EVENTS", "BULK_DELETE_EVENT", "CUT_SELECTION", "PASTE_SELECTION", "SWITCH_EVENT_COLOR", "NUDGE_SELECTION"]),
...
//// SELECTORS
//
export const getTracks = (state) => state.editorEntities.eventsView.present.tracks;
...
export const getEvents = createSelector(getTracks, (tracks) => flatten(Object.values(tracks)));
...
export const makeGetEventsForTrack = (trackId) =>
	createSelector(getStartAndEndBeat, getTracks, ({ startBeat, endBeat }, tracks) => {
